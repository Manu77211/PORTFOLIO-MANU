...
export const LogoLoop = memo(
  ({
    ...
    const renderLogoItem = useCallback(
      (item: LogoItem, key: string) => {
        if (renderItem) {
          return (
            <li 
              ...
              className={scaleOnHover ? 'overflow-visible group' : ''}
            >
              {renderItem(item, key)}
            </li>
          );
        ...
        const content = isNodeItem ? (
          <span 
            className={`inline-flex items-center transition-transform duration-300 ${scaleOnHover ? 'group-hover:scale-110' : ''}`}
          ...
          <img
            ...
            className={`block transition-transform duration-300 ${scaleOnHover ? 'group-hover:scale-110' : ''}`}
          />
        );
        ...
        const itemContent = item.href ? (
          <a
            ...
            className="inline-flex items-center no-underline rounded transition-opacity duration-200 hover:opacity-80 focus-visible:outline focus-visible:outline-2 focus-visible:outline-current focus-visible:outline-offset-2"
          >
            {content}
          </a>
        ) : (
    ...
    const containerStyle: React.CSSProperties = useMemo(
      () => ({
        position: 'relative',
        width: isVertical ? (width === '100%' ? undefined : width) : width,
        ...
        {fadeOut && (
          <>
            <div 
              ...
              style={{
                width: isVertical ? '100%' : 'clamp(24px, 8%, 120px)',
                height: isVertical ? 'clamp(24px, 8%, 120px)' : '100%',
